...
public class BagRepository {
	...
	public Integer save(Bag bag){
		...
		System.out.println("insert into bag (type_id, name, description, price) values (" + bag.getType().getId()
		...
		jdbc.update("insert into bag (type_id, name, description, price) values (" + bag.getType().getId()
				+ ", '" + bag.getName() + "' , '" + bag.getDescription() + "', " + bag.getPrice() + ")");
		return jdbc.queryForObject("select max(id) latest_id from bag",
	...
	public List<Bag> findAll(){
		return jdbc.query("select * from ( bag join bag_type on bag_type.id = bag.type_id)", 
				(ResultSet rs, int rowNum) -> {return new Bag(rs.getLong("id"),
						new Type (rs.getString("name"), rs.getLong("type_id")),
	...
	public List<Bag> findByType(Type type){
		...
		return jdbc.query("select * from bag where type_id = " + type.getId(), 
